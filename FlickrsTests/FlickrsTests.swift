...
import XCTest
@testable import Flickrs

class FlickrsTests: XCTestCase {
...
//    }
    ...
    func testSearchValidText() {
        ...
        MainServices.shared.request("dogs") { (result) in
    ...
    func testValidatePhotoImageURL() {
        ...
        HomeServices.shared.request("dogs",pageNo: 1) { (result) in
    ...
    func testSearchInvalidText() {
        ...
        HomeServices().request("", pageNo: 1) { (result) in
            switch result {
            case .Success( _):
                XCTFail("No results")
            case .Failure( _):
